...
if run_as_static:
    if isDynamic:
        ...
        onnx.save(model, "../../../../onnx/models/opt-125m-awq/dynamic_model.onnx")
        print("\t dynamic model saved as dynamic_model.onnx")

        print("Setting model dim_params:")
        for p, v in zip(dim_params, dim_values):
    ...
    try:
        model = onnx.load("../../../../onnx/models/opt-125m-awq/dynamic_model.onnx")
        print("\t dynamic model found! \nSaving dynamic model as model.onnx...")
        onnx.save(model, "model.onnx")
        print("\t dynamic model saved as model.onnx.")
        os.remove("../../../../onnx/models/opt-125m-awq/dynamic_model.onnx")
    except OSError as e:
        print(e, "\nNo dynamic_model found. Running static model instead.")

# get some model inputs
model_inputs = [
    numpy.random.rand(pv["batch_size"], pv["sequence_length"]).astype(numpy.int64)
]  # input_ids
model_inputs.append(
    numpy.random.rand(pv["batch_size"], pv["past_sequence_length + 1"]).astype(
        numpy.int64
    )
)  # attention_mask
for i in range(2 * 12):
